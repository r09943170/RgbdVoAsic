...
int main(int argc, char** argv)
{
    ...
    char dlmrt = '/';
    size_t pos = filename.rfind(dlmrt);  // 由後往前找filename字串中的"/"字符
    string dirname = pos == string::npos ? "" : filename.substr(0, pos) + dlmrt; //nops==-1，表沒有找到位置
    ...
    Mat cameraMatrix = Mat::eye(3,3,CV_64FC1);  //Mat == Matrix; eye == diagonal matrix, 預設為單位矩陣I; CV_64FC1 == 倍精度float, channel數=1
    {
    ...
    Mat sigma_debug;
    sigma_debug = Mat(2, 2, CV_64FC1, Scalar(1));
    // sigma_debug.at<double>(0,0) = 7774054188783870;
    sigma_debug.at<double>(0,0) = 0; //icp
    sigma_debug.at<double>(1,1) = 0; //rgbd
    ...
    for(int i = 0; !file.eof(); i++)
    {
...

        {
            ...
            if(!Rts.empty())
            {
                ...
                tm.start();
                gtm.start();
                bool res = odometry.compute(frame_curr, frame_prev, Rt, v_max, sigma_debug);
                gtm.stop();
                tm.stop();
                count++;
