                       ...
                       Mat& debug) //pylin
{
    ...
    bool check_near_1to2 = if_1.good();
    if (check_near_0to1 == 0) {
        // ofstream of_0("./debug_check_proj_d1_SW.txt");
    int store_index = 32;
    int load_v_origin = 0;
                         ...
                         int transformDim)  //transformDim = 6
{
    ...
    ifstream if_1("./debug_check_dI_dx_SW.txt");
    bool check_if_1 = if_1.good();
    if (check_if_1 == 0) {
    ofstream of_1("./debug_check_dI_dx_SW.txt");
    for(int correspIndex = 0; correspIndex < corresps.rows; correspIndex++)
    {
         const Vec4i& c = corresps_ptr[correspIndex];
         int u0 = c[0], v0 = c[1];  //src
         ...
         of_1 << dI_dx1.at<short int>(v1,u1) << endl;   //dI_dx
        //  of_1 << dI_dy1.at<short int>(v1,u1) << endl;   //dI_dy
...
bool Odometry::compute(Ptr<OdometryFrame>& srcFrame, Ptr<OdometryFrame>& dstFrame, Mat& Rt, int& v_max, Mat& sigma_debug, const Mat& initRt) const
{
    ...
    bool isOk = false;
    {
        ...
        for(int iter = 0; iter < iterCounts_vec[0]; iter ++)  //pylin test
        {   
                ...
                if(corresps_rgbd.rows >= minCorrespsCount)
                {
                    ...
                    for (int i = 0; i < transformDim; i++){
                        for (int j = 0; j < transformDim; j++){
                            AtA_vec[i][j] += AtA_rgbd.at<double>(i,j);
                            // cout << "AtA_rgbd[" << i << "][" << j << "] = " << AtA_vec[i][j] << endl;
                        }
                        AtB_vec[i] += AtB_rgbd.at<double>(i,0);
                    }
                }
